...
from src.config import settings, PROCESSED_DATA_DIR, REPORTS_DIR
...
class WeatherDataTransformer:
    ...
    def create_lag_features(
        self,
        df: pd.DataFrame,
        columns: list[str] = ["temperature", "humidity", "pressure", "wind_speed"],
    ...
    def create_diff_features(
        self,
        df: pd.DataFrame,
        columns: list[str] = ["temperature", "humidity", "pressure"],
    ) -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    sample_data = pd.DataFrame({
        ...
        "temperature": np.random.uniform(10, 30, 200) + np.sin(np.arange(200) * 2 * np.pi / 24) * 5,
